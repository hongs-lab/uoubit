...
const KO_SUFFIX = [
  '전자', '중공업', '건설', '화학', '산업', '정밀', '소재', '물산', '상사',
  '제강', '정공', '전선', '기연', '중공', '홀딩스', '금융지주', '캐피탈',
  '개발', '엔지니어링', '테크', '시스템', '에너지', '통상', '실업', '공업',
];

const EN_SUFFIX = [
  '테크놀로지', '랩스', '다이내믹스', '솔루션즈', '네트웍스', '시스템즈',
  '바이오', '헬스케어', '로보틱스', '모빌리티', '마이크로', '세미콘',
  '코퍼레이션', '파트너스', '인더스트리', '소재산업', '일렉트릭',
  '이노베이션', '인터내셔널', '글로벌', '파워', '미디어', '컨설팅', '애셋',
  '트러스트',
];

const LATIN_HEAD = [
  'NEX', 'VEL', 'ZEN', 'ORB', 'LUM', 'KOR', 'ARC', 'TRI', 'AXI', 'QUA',
  'HEL', 'VER',
];
...
function main() {
  ...
  for (const l of listed) {
    ...
    let name = '';
    if (n % 5 === 0) {
      for (let i = 0; i < LATIN_HEAD.length * LATIN_TAIL.length && !name; i += 1) {
